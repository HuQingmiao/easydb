...
public class Exp {
	...
	public static void main(String[] args) {
		// gender = 'F' and (status=1 or (status=2 and name like '%��')) and type
		// in (2,3)

		// Criterion c1 = Expression.eq("gender","F");
		//
		// Criterion c2 =
		// Expression.and(Expression.eq("status","2"),Expression.like("status","%��"));
		//
		// Expression.or(c1,c2);
		//
		// Expression.eq("status",1).or(Expression.eq("status",2).and(Expression.like("status","%��")))
		//

		try {

			// // ��������"��Զ��UNIX"��ͷ���Ҽ۸�>=70.0Ԫ������и���
			// Criteria c1 = Exp.like("title", "��Զ��UNIX%");
