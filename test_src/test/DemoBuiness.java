...
import walker.easydb.criterion.Exp;
...
public class DemoBuiness {
	...
	public void updateBookByCriteria() throws Exception {	    
		...
		boolean commit = false;// 事务执行成功的标识
		try {
			// 设置要更新的项:价格,书名,书的二进制内容,书的文本内容
			Book newBookInfo = new Book();
			newBookInfo.setCost(new EFloat("65.0"));// 更新价格
			newBookInfo.setTitle(new EString("永远的UNIX_上册(高人篇)"));

			// 开放此代码,将产生异常:对BLOB/CLOB的写入，每次只能作用在用主键值标识的唯一记录上！
			 //newBookInfo.setBlobContent(new EBinFile("d:\\unix.chm"));
			// newBookInfo.setTextContent(new ETxtFile("d:\\unix.txt"));

			// 如果要将大字段列置空,则..
			// newBookInfo.setBlobContent(new EBinFile());//以二进制方式更新内容
			// newBookInfo.setTextContent(new ETxtFile());//以文本方式更新内容

			// 对名称以"UNIX"开头的且价格>=70.0元的书进行更新
...
//			           + " VALUES (3, '鲁宾逊漂流记', 20.0,TO_DATE('2007-01-10', 'yyyy-MM-dd')) ";
...
//			 }
			...
			//this.executeSqlToAddBookEditor();//第六个子事务
	...
	public void addMultiBooks2() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                BookArray[i].setTitle(new EString("UNIX-上册"+i));
